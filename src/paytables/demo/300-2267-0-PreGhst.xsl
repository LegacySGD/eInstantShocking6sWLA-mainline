			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						const symbolWheel 	  = 'FCHDABGFEHBCAGECBDFHADGE';
						...
						const colourPurple = '#b400fd'; 
						const colourRed    = '#fe3439'; 
						const colourWhite  = '#ffffff';  
						const colourYellow = '#ffff3d'; 
						const colourCardBack = '#563caa'; 

						const prizeColours       	  = [colourBlue, colourNavy, colourGreen, colourLime, colourRed, colourPurple, colourYellow, colourOrange];
						...
						function showSymb(A_strCanvasId, A_strCanvasElement, A_strBoxColour, A_strTextColour, A_strText, A_iBoxSize)
						{
						...
						function showGridSymbs(A_strCanvasId, A_strCanvasElement, A_arrGrid, A_arrPrize, A_arrWins)
						{
							...
							for (var gridRow = 0; gridRow < gridRows; gridRow++)
							{
								for (var gridCol = 0; gridCol < gridCols; gridCol++)
								{
									...
									{
										cellVal   = temp.charCodeAt(0) - 'A'.charCodeAt(0) + 1;
										cellVal    += (cellVal >= 6) ? 1 : 0;
										symbCell   = cellVal.toString();
										boolIWCell = false;
									}
						...
						for (var prizeIndex = 1; prizeIndex < arrCellBackgroundColour.length; prizeIndex++)
						{
							...
							showSymb(canvasIdStr, elementStr, boxColourStr, colourBlack, "", smCellSize);
						...
						showSymb(canvasIdStr, elementStr, boxColourStr, colourWhite, "B", smCellSize);
						...
						for (var luckyIndex = 0; luckyIndex < winLetters.length; luckyIndex++)
						{
							...
							boxColourStr = colourPink;
							cellVal      = symbLetter.charCodeAt(0) - 'A'.charCodeAt(0) + 1;
							cellVal      += (cellVal >= 6) ? 1 : 0;

							r.push('<td>');
							showSymb(canvasIdStr, elementStr, boxColourStr, colourBlack, cellVal.toString(), cellSizeWinSymb);
							r.push('</td>');
						}
						...
						for (var gameIndex = 0; gameIndex < orderedWins.length; gameIndex ++)
						{
							...
							if (temp == 1)
							{
								showSummarySymb(canvasIdStr, elementStr, boxColourStr, textColourStr, cellVal);
							}
							else if (temp == 2)
							{
								triggerCount = 0;
